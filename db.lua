if DB_LOADED and not _G.DB_DEBUG == true then
    -- error("Dragon Blox is already running!", 0)
    return
end

pcall(function() getgenv().DB_LOADED = true end)
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Dragon Blox is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Dragon Blox FE v" .. currentVersion
...
Logo.Image = "rbxassetid://12634936615"
...
About.TextSize = 14
About.Text = "Plugins are .db files and should be located in the 'workspace' folder of your exploit."
...
DBMouse = Players.LocalPlayer:GetMouse()
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with DB,  and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
...
function saves()
    if writefileExploit() and readfileExploit() and jsonAttempts < 10 then
        local readSuccess, out = readfile("DB_FE.db")
        if readSuccess then
            if out ~= nil and tostring(out):gsub("%s", "") ~= "" then
                local success, response = pcall(function()
                    ...
                    if json.keepDB ~= nil then KeepInfYield = json.keepDB else KeepInfYield = true end
                ...
                if not success then
                    ...
                    writefileCooldown("DB_FE.db", defaults)
                    wait()
                    saves()
                end
            else
                writefileCooldown("DB_FE.db", defaults)
                wait()
                local dReadSuccess, dOut = readfile("DB_FE.db")
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("DB_FE.db", HttpService:JSONEncode(update))
	end
end
...
DBMouse.KeyDown:Connect(function(Key)
	if (Key==prefix) then
		Cmdbar:CaptureFocus()
		spawn(function()
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Dragon Blox Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
function selectPart()
	...
	local function HighlightPart()
		if selected.Adornee ~= DBMouse.Target then
			selectionBox.Adornee = DBMouse.Target
		else
			selectionBox.Adornee = nil
		end
	end
	ActivateHighlight = DBMouse.Move:Connect(HighlightPart)
	local function SelectPart()
		if DBMouse.Target ~= nil then
			selected.Adornee = DBMouse.Target
			Path.Text = getHierarchy(DBMouse.Target)
		end
	end
	ClickSelect = DBMouse.Button1Down:Connect(SelectPart)
end
...
CMDs[#CMDs + 1] = {NAME = 'olddex / odex', DESC = 'Opens Old DEX'}
...
CMDs[#CMDs + 1] = {NAME = 'hidedb', DESC = 'Hides the main DB GUI'}
CMDs[#CMDs + 1] = {NAME = 'showdb / unhidedb', DESC = 'Shows DB again'}
CMDs[#CMDs + 1] = {NAME = 'keepdb', DESC = 'Auto execute DB when you teleport through servers'}
CMDs[#CMDs + 1] = {NAME = 'unkeepdb', DESC = 'Disable keepdb'}
CMDs[#CMDs + 1] = {NAME = 'togglekeepdb', DESC = 'Toggle keepdb'}
...
function checkTT()
	local t
	local guisAtPosition = COREGUI:GetGuiObjectsAtPosition(DBMouse.X, DBMouse.Y)
	...
	if t ~= nil and t:GetAttribute("Title") ~= nil then
		...
		local xP
		local yP
		if DBMouse.X > 200 then
			xP = x - 201
		else
			xP = x + 21
		end
		if DBMouse.Y > (DBMouse.ViewSizeY-96) then
			yP = y - 97
		else
			yP = y
		end
			...
			if cmd then
				...
				if infTimes then
					while lastBreakTime < cmdStartTime do
						...
						if not success and _G.DB_DEBUG then
...
local MousePositionToVector2 = function()
	return Vector2.new(DBMouse.X, DBMouse.Y)
end
...
local function clicktpFunc()
	pcall(function()
		...
		local hitPosition = DBMouse.Hit.Position
...
DBMouse.Button1Down:Connect(function()
	for i,v in pairs(binds) do
		...
		elseif v.COMMAND == 'clickdel' then
			...
			if input == 'RightClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
				pcall(function() DBMouse.Target:Destroy() end)
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'db_fe.db' or name == 'db_fe' then
		notify('Plugin Error','Please enter a valid plugin')
	else
		local file
		local fileName
		if name:sub(-3) == '.db' then
			pcall(function() file = readfile(name) end)
			fileName = name
		else
			pcall(function() file = readfile(name..'.db') end)
			fileName = name..'.db'
		end
		if file then
...
addcmd('keepdb', {}, function(args, speaker)
...
addcmd('unkeepdb', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = false
		updatesaves()
	else
...
addcmd('togglekeepdb', {}, function(args, speaker)
	if queueteleport then
...
function sFLY(vfly)
	...
	flyKeyDown = DBMouse.KeyDown:Connect(function(KEY)
		if KEY:lower() == 'w' then
			CONTROL.F = (vfly and vehicleflyspeed or dbflyspeed)
		elseif KEY:lower() == 's' then
			CONTROL.B = - (vfly and vehicleflyspeed or dbflyspeed)
		elseif KEY:lower() == 'a' then
			CONTROL.L = - (vfly and vehicleflyspeed or dbflyspeed)
		elseif KEY:lower() == 'd' then 
			CONTROL.R = (vfly and vehicleflyspeed or dbflyspeed)
		elseif QEfly and KEY:lower() == 'e' then
			CONTROL.Q = (vfly and vehicleflyspeed or dbflyspeed)*2
		elseif QEfly and KEY:lower() == 'q' then
			CONTROL.E = -(vfly and vehicleflyspeed or dbflyspeed)*2
		end
		pcall(function() workspace.CurrentCamera.CameraType = Enum.CameraType.Track end)
	end)
	flyKeyUp = DBMouse.KeyUp:Connect(function(KEY)
		if KEY:lower() == 'w' then
			CONTROL.F = 0
		elseif KEY:lower() == 's' then
			CONTROL.B = 0
	...
	mfly2 = RunService.RenderStepped:Connect(function()
		...
		if speaker.Character:FindFirstChildWhichIsA("Humanoid") and root and root:FindFirstChild(velocityHandlerName) and root:FindFirstChild(gyroHandlerName) then
			...
			if direction.X > 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * ((vfly and vehicleflyspeed or dbflyspeed) * 50))
			end
			if direction.X < 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * ((vfly and vehicleflyspeed or dbflyspeed) * 50))
			end
			if direction.Z > 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * ((vfly and vehicleflyspeed or dbflyspeed) * 50))
			end
			if direction.Z < 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * ((vfly and vehicleflyspeed or dbflyspeed) * 50))
			end
...
addcmd('float', {'platform'},function(args, speaker)
	...
	if pchar and not pchar:FindFirstChild(floatName) then
		task.spawn(function()
			...
			qUp = DBMouse.KeyUp:Connect(function(KEY)
				if KEY == 'q' then
					FloatValue = FloatValue + 0.5
				end
			end)
			eUp = DBMouse.KeyUp:Connect(function(KEY)
				if KEY == 'e' then
					FloatValue = FloatValue - 0.5
				end
			end)
			qDown = DBMouse.KeyDown:Connect(function(KEY)
				if KEY == 'q' then
					FloatValue = FloatValue - 0.5
				end
			end)
			eDown = DBMouse.KeyDown:Connect(function(KEY)
				if KEY == 'e' then
					FloatValue = FloatValue + 0.5
				end
			end)
...
function deleteGuisAtPos()
	pcall(function()
		local guisAtPosition = Players.LocalPlayer.PlayerGui:GetGuiObjectsAtPosition(DBMouse.X, DBMouse.Y)
		for _, gui in pairs(guisAtPosition) do
...
addcmd('hidedb',{},function(args, speaker)
	isHidden = true
	...
	minimizeNum = 0
	minimizeHolder()
	if not (args[1] and tostring(args[1]) == 'nonotify') then notify('DB Hidden','You can press the prefix key to access the command bar') end
end)

addcmd('showdb',{'unhidedb'},function(args, speaker)
	isHidden = false
	minimizeNum = -20
	if wasStayOpen then
		maximizeHolder()
...
addcmd('tptool', {'teleporttool'}, function(args, speaker)
	...
	TpTool.Activated:Connect(function()
		...
		HRP.CFrame = CFrame.new(DBMouse.Hit.X, DBMouse.Hit.Y + 3, DBMouse.Hit.Z, select(4, HRP.CFrame:components()))
	end)
end)
...
addcmd('hovername',{},function(args, speaker)
	...
	nbUpdateFunc = DBMouse.Move:Connect(updateNameBox)
end)
...
DBMouse.Move:Connect(checkTT)

task.spawn(function()
	local success, latestVersionInfo = pcall(function() 
		local versionJson = game:HttpGet('https://raw.githubusercontent.com/EdgeDB/infiniteyield/master/version')
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Dragon Blox may have issues or features that are not functioning correctly.") end
end)
